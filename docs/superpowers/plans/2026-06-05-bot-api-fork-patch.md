...
**Goal:** Гілка `custom` у `MEHMMETT/telegram-bot-api` з патчем (getMessages, getUserInfo, --allowed-bot-ids, --relative, --stats-hide-sensible-data) і GitHub Actions, що щодня ребейзить її на upstream, збирає Docker-образ і деплоїть через Coolify webhook.
...
**Контекст:** Спек — `docs/superpowers/specs/2026-06-05-custom-bot-api-server-design.md` (MEHMMETT_Quotely). Робоча копія форка: `/Users/ly/dev/telegram-bot-api-fork` (зараз shallow — Task 1 переклонує). Перевірки виконуються **тестовим** бот-токеном (`$TEST_BOT_TOKEN`), не продовим: перший запит до локального сервера розлогінює токен із хмари.
...
git clone --recursive https://github.com/MadebyMehmmeTT/telegram-bot-api.git /Users/ly/dev/telegram-bot-api-fork
    ...
    steps:
      ...
      - uses: docker/build-push-action@v6
        with:
          ...
          push: true
          tags: |
            ghcr.io/MadebyMehmmeTT/telegram-bot-api:latest
            ghcr.io/MadebyMehmmeTT/telegram-bot-api:${{ steps.ver.outputs.version }}
...
Очікувано: образ `ghcr.io/MadebyMehmmeTT/telegram-bot-api:latest` опублікований.
...
**Наступні плани:** №2 — Coolify-деплой (compose: bot-api + caddy сайдкар, домени, volume, secrets, перенесення вебхука в Actions), №3 — міграція MEHMMETT_Quotely (BOT_API_ROOT, видалення `src/services/tdlib/`, новий fetcher, оновлення quote-api). Пишуться після виконання цього.
