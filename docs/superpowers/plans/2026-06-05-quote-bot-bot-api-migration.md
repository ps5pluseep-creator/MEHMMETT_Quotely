# MEHMMETT_Quotely → Custom Bot API Server Migration Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Remove in-process TDLib (`tdl` + `prebuilt-tdlib`) from MEHMMETT_Quotely and use the self-hosted fork (https://tg-api.yuri.ly, MEHMMETT/telegram-bot-api) as the Bot API root for everything, including the two custom methods `getMessages` / `getUserInfo`.
...
/**
 * Thin client for the custom Bot API server methods (MEHMMETT/telegram-bot-api):
...
4. **Switch the bot.** Prod MEHMMETT_Quotely env: set
