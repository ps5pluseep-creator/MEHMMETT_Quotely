...
const ME = { id: 42, username: 'MEHMMETT_Quotelybot' }
...
describe('isRelevantUpdate', () => {
  ...
  it('accepts an @mention of the bot in a group', () => {
    const c = ctx({
      chat: { type: 'supergroup' },
      message: { text: 'hey @MEHMMETT_Quotelybot', entities: [{ type: 'mention', offset: 4, length: 10 }] },
    })
    expect(isRelevantUpdate(c)).toBe(true)
  })
  ...
  it('accepts a mention typed in a different case than the username', () => {
    const c = ctx({
      me: { id: 42, username: 'MEHMMETT_Quotely_bot' },
      chat: { type: 'supergroup' },
      message: { text: '@MEHMMETT_Quotelybot', entities: [{ type: 'mention', offset: 0, length: 10 }] },
    })
    expect(isRelevantUpdate(c)).toBe(true)
  })
